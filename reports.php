...
/**
 * reports.php
 * Facilitates the generation of comprehensive attendance reports for Faculty Interns.
 * Utilises the Singleton database connection to aggregate data across multiple course modules.
 */
...
/**
 * Establishment of the database connection utilising the Singleton instance.
 * This ensures the report generator uses the centralised 'geospacial_attendance_management' repository.
 */
$conn = connectDB();
...
/**
 * SQL execution to retrieve attendance records, student metadata, and session identifiers.
 * The results are ordered by date and course code for systematic review.
 */
$sql = "
    SELECT
        a.attended_at,
...
try {
    ...
    // The connection closure is handled by the Singleton lifecycle if necessary,
    // but explicit closure here ensures immediate resource release for large reports.
    if (isset($conn) && $conn instanceof mysqli) {
        $conn->close();
    }
}
...
/**
 * Helper function to transform status codes into professional descriptors.
 * @param string $status
 * @return string
 */
function get_status_label($status) {
